...
public class VideoDecoderCore {
     ...
     * 获取视频信息和配置decoder
     */
    private VideoDecoderCore() {
    }

    public static VideoDecoderCore EMPTY() {
        return new VideoDecoderCore();
    }

    public static VideoDecoderCore just(File srcFile) {
        VideoDecoderCore videoDecoderCore = new VideoDecoderCore();
        videoDecoderCore.setSourceFile(srcFile);
    ...
    public void setFrameCallback(FrameCallback mFrameCallback) {
    ...
    public void setSourceFile(File srcFile) {
        ...
        try {
            ...
            //找到Video Track
            int videoTrackIndex = selectTrack(extractor);
            if (videoTrackIndex < 0) {
                throw new RuntimeException("No video track found in " + srcFile.getAbsolutePath());
            }
            //选中对应track
    ...
    public static class PlayTask implements Runnable {
        ...
        private VideoDecoderCore mPlayer;
         ...
         */
        public PlayTask(VideoDecoderCore player, PlayerFeedback feedback) {
            mPlayer = player;
            mFeedback = feedback;
