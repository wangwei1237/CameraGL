...
public class DecodeThread extends HandlerThread implements VideoDecoderCore.PlayerFeedback {
    ...
    private WeakReference<CameraView> viewRef;
    private VideoDecoderCore.PlayTask mPlayTask;

    public DecodeThread(CameraView captureFilterView) {
        super("DecodeThread");
        start();
        mVideoDecoder = VideoDecoderCore.EMPTY();
        this.viewRef = new WeakReference<>(captureFilterView);
        mDecodeHandler = new DecodeHandler(getLooper(), new WeakReference<DecodeThread>(this));
    }
...
//        mPlayTask.setLoopMode(true);
    ...
    private void setSourceFile(File src) {
        ...
        //设置好之后，要回调设置长宽
        CameraView videoCaptureFilterView = viewRef.get();
    ...
    private final static class DecodeHandler extends Handler {
        private final WeakReference<DecodeThread> ref;


        public DecodeHandler(Looper looper, WeakReference<DecodeThread> decodeThread) {
            super(looper);
        ...
        public void handleMessage(Message msg) {
            DecodeThread decodeThread = ref.get();
